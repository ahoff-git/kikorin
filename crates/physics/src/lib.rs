...
impl PhysicsWorld {
    ...
    pub fn sync_from_world(&mut self, world: &World) {
        for id in world.entities() {
            ...
            if let Some(&rb_handle) = self.entity_to_rb.get(&id) {
                // Fixed bodies never move — skip the per-tick translation update entirely.
                // Previously set_translation was called even for floor tiles (with wake_up=true),
                // forcing unnecessary broadphase work in Rapier every tick.
                if is_dynamic {
                ...
                let col_builder = if cfg.sensor {
                    ...
                    // Average=0). solver_groups limits force resolution to GROUP_1 (floor/walls)
                    // so agents don't physically block each other; collision_groups stays default
                    // so contact events are still generated and touching() remains accurate.
                    ColliderBuilder::new(shape)
                        .friction(0.0)
                        .friction_combine_rule(CoefficientCombineRule::Multiply)
                        .solver_groups(InteractionGroups::new(Group::GROUP_2, Group::GROUP_1))
                } else {
                    ColliderBuilder::new(shape)
                };
                let col_handle =
    ...
    pub fn sync_to_world(&self, world: &mut World) {
        for (&id, &rb_handle) in &self.entity_to_rb {
            ...
            let grounded = if let Some(cfg) = world.collider(id) {
