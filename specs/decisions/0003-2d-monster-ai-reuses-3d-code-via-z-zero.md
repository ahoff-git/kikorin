...
- **Known gap, deliberately deferred**: `MonsterState` has no multi-jump budget the way `PlayerState` does (`PlayerConfig.max_jumps`) — monsters can only take one jump per waypoint trigger. The 2D player can double-jump; monsters currently can't. The navmesh is built assuming the monsters' real (single-jump) capability, so they route around anything needing a second jump rather than get stuck attempting it. Giving monsters a real jump budget is a natural, separate follow-up.
- **Also deliberately deferred**: per-monster-type capability (agile/slow/flying archetypes) — `AiConfig` is one engine-global instance, not per-monster, for both dimensions. That's a bigger structural change to `tick_monster_ai`/`MonsterState`, out of scope here, and now would benefit both 2D and 3D uniformly instead of needing a separate TS-side system.
