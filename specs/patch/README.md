...
- `SemanticPatch` — emitted for entities with `DirtyFlags::HEALTH`, `NET`, or `ANIM`. Carries `health`/`net_flags`/`grounded` plus the animation cell (`anim_id`/`anim_frame`/`anim_dir`, present when ANIM is dirty) — the engine's animation state machine's per-tick output, which TS displays and never recomputes (ADR 0015).
